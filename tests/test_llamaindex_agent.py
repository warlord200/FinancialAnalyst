"""Legacy retrieval-quality eval for the LlamaIndex agent.

This file is a manual eval script, not part of the routine test suite. It
loads real embedding models and LLMs, builds per-company indexes, generates
QA datasets, and evaluates retrieval quality. Running it takes a very long
time, so the heavy work only runs when the module is executed directly:

    python tests/test_llamaindex_agent.py

Under pytest the module imports quickly and collects no tests; the eval
harness that replaces it is T13.
"""
