...
from tests.fixtures.steps import FakeLLM, FakeRetriever, make_financials_corpus, valid_financials_section_json
from tests.fixtures.xbrl_facts import make_facts


def make_financials_client(tmp_path, monkeypatch, llm=None, retriever=None, refresh=True):
    ...
    retriever = retriever or FakeRetriever(make_financials_corpus())
    llm = llm or FakeLLM(valid_financials_section_json())
...
class TestFinancials:
    def test_financials_renders_tables_and_forensic_sections(self, tmp_path, monkeypatch):
        client, _, _ = make_financials_client(tmp_path, monkeypatch)
        resp = client.get("/api/steps/TSLA/financials")
        assert resp.status_code == 200
        body = resp.json()
        assert body["ticker"] == "TSLA"
        assert body["cached"] is False
        artifact = body["artifact"]
        assert artifact["artifact_type"] == "financials"
        assert artifact["fiscal_year"] == 2025
        assert artifact["scope"]["items"] == ["ITEM 7", "ITEM 8"]
        table_keys = [t["key"] for t in artifact["tables"]]
        assert "common_size_income" in table_keys
        assert "margins" in table_keys
        assert "liquidity_solvency" in table_keys
        assert "cagr" in table_keys
        margin_row = next(
            r for t in artifact["tables"] if t["key"] == "margins"
            for r in t["rows"] if r["label"] == "Gross margin"
        )
        assert margin_row["values"]["2025"] == pytest.approx(0.25)
        assert {"type": "xbrl_fact", "value": "GrossProfit"} in margin_row["sources"]
    ...
    def test_financials_is_cached_after_first_generation(self, tmp_path, monkeypatch):
        client, _, llm = make_financials_client(tmp_path, monkeypatch)
        first = client.get("/api/steps/TSLA/financials")
        assert first.status_code == 200
        assert first.json()["cached"] is False
        calls_after_first = llm.calls
        second = client.get("/api/steps/TSLA/financials")
        assert second.status_code == 200
        assert second.json()["cached"] is True
        assert second.json()["artifact"] == first.json()["artifact"]
        assert llm.calls == calls_after_first

    def test_financials_404_when_numbers_missing(self, tmp_path, monkeypatch):
        client, _, _ = make_financials_client(tmp_path, monkeypatch, refresh=False)
        assert client.get("/api/steps/TSLA/financials").status_code == 404

    def test_financials_404_when_not_ingested(self, tmp_path, monkeypatch):
        client, _, _ = make_financials_client(
            tmp_path, monkeypatch, retriever=FakeRetriever(chunks=[], years=[])
        )
        assert client.get("/api/steps/TSLA/financials").status_code == 404

    def test_financials_requires_auth(self, tmp_path, monkeypatch):
        client, _, _ = build_client(tmp_path, monkeypatch)
        assert client.get("/api/steps/TSLA/financials").status_code == 401

    def test_financials_502_when_llm_cannot_ground(self, tmp_path, monkeypatch):
        client, _, _ = make_financials_client(
    ...
    def test_financials_503_on_embedding_model_mismatch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            main,
            "_get_steps_service",
            lambda: SimpleNamespace(financials=boom),
        )
        client, _, _ = build_client(tmp_path, monkeypatch, limits=GENEROUS_LIMITS)
        signup_and_auth(client, email="steps@example.com")
        resp = client.get("/api/steps/TSLA/financials")
