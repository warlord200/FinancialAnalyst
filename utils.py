...
from llama_index.core import Settings, VectorStoreIndex, SimpleDirectoryReader, Document, StorageContext, SummaryIndex, load_index_from_storage
...
class CustomDocs:
    ...
    def _create_collection(self):
        ...
        return v_chroma_collection, v_chroma_collection.count() , s_chroma_collection, s_chroma_collection.count()
    ...
    def _build_index(self,path):
        ...
        if self._v_collection_size == 0:
            ...
            documents = SimpleDirectoryReader(
                input_dir=path,
                file_extractor=self._extractor
                ).load_data()
            ...
            summ_idx = SummaryIndex.from_documents(documents, storage_context=s_storage_context)
            summ_idx.storage_context.persist(persist_dir=self._persist_dir)
        else:
            print("Found file in collection, skipped parsing")
            vec_idx = VectorStoreIndex.from_vector_store(v_store, storage_context = v_storage_context)

            s_storage_context = StorageContext.from_defaults(vector_store=s_store, persist_dir=self._persist_dir)
    ...
    def get_tools(self) -> list[QueryEngineTool]:
        ...
        s_query_eng = self.summ_idx.as_query_engine(
